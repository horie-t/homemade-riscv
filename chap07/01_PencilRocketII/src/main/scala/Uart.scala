...
class UartTx extends Module {
  ...
  val (txCount, txEnable) = Counter(true.B, clock_freq / trans_bps) // 状態遷移用カウンタ
...
class UartRx extends Module {
  ...
  val (rxCount, rxEnable) = Counter(true.B, clock_freq / trans_bps) // 状態遷移用カウンタ
  val (rxHalfCount, rxHalfEnable) = Counter(true.B, clock_freq / trans_bps / 2) // 1/2ビット周期カウンタ
